...
import (
	...
	"github.com/candy12t/go-blockchain/utils"
...
func NewWallet() *Wallet {
	...
	h3.Write(digest2)
	digest3 := h2.Sum(nil)
	// 4. Add version bytes in front of RIPEMD-160 hash (0x00 for Main Newwork) .
	vd4 := make([]byte, 21)
	vd4[0] = 0x00
	copy(vd4[1:], digest3[:])
	...
	h5.Write(vd4)
	digest5 := h5.Sum(nil)
	// 6. Perform SHA-256 hash on the result of the previoush SHA-256 hash .
	h6 := sha256.New()
	h6.Write(digest5)
	digest6 := h6.Sum(nil)
	// 7. Take the first 4 bytes of the second SHA-256 hash for checksum .
	chsum := digest6[:4]
	// 8. Add the 4 checksum bytes from 7 at the end of extended RIPEMD-160 hash from 4 (25 bytesa) .
	dc8 := make([]byte, 25)
	copy(dc8[:21], vd4[:])
	copy(dc8[21:], chsum[:])
